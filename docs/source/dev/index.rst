...
The Obsidian-Sync project implements a regression
...
not broken existing functionality. The Obsidian-Sync
...
.. code-block:: console
    ...
    (obsidian-sync) $ make test

To run tests verbosely use:

.. code-block:: console

    (obsidian-sync) $ make test-verbose
...
``obsidian-sync`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``obsidian-sync`` package can be found.

.. code-block:: console

    (obsidian-sync) $ cd tests
    (obsidian-sync) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (obsidian-sync) $ python -m test_version
...
.. code-block:: console
    ...
    (obsidian-sync) $ make sort-imports
...
.. code-block:: console
    ...
    (obsidian-sync) $ make check-static-analysis
...
.. code-block:: console
    ...
    (obsidian-sync) $ make check-types
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/obsidian-sync/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (obsidian-sync) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (obsidian-sync) $ make dist-upload

  The package should now be available at https://pypi.org/project/obsidian-sync/
