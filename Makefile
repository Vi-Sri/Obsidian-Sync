...
# help:
# help: Obsidian-Sync Makefile help
# help:
...
venv:
	@rm -Rf venv
	@python3 -m venv venv --prompt obsidian-sync
...
.PHONY: format
format:
	@black src/obsidian-sync tests examples


# help: check-format                   - check code format compliance
.PHONY: check-format
check-format:
	@black --check src/obsidian-sync tests examples
...
.PHONY: check-types
check-types:
	@cd src; mypy -p obsidian-sync --ignore-missing-imports


# help: check-lint                     - run static analysis checks
.PHONY: check-lint
check-lint:
	@pylint --rcfile=.pylintrc obsidian-sync ./tests setup.py ./examples
...
.PHONY: docs
docs: coverage
	@cd docs; rm -rf source/api/obsidian-sync*.rst source/api/modules.rst build/*
	@cd docs; make html
...
.PHONY: dist-test
dist-test: dist
	@cd dist && ../tests/test-dist.bash ./obsidian-sync-*-py3-none-any.whl


# help: dist-upload                    - upload a wheel distribution package
.PHONY: dist-upload
dist-upload:
	@twine upload dist/obsidian-sync-*-py3-none-any.whl
